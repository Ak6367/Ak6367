...
$getdata = $conn->query("select * from `city` where `status` = 1 && dstatus = 0 ".$searchvar."  order by id desc LIMIT $start_from, $per_page_record ");
// print_r($getdata);die;

$chekdata =$conn->query("select * from `city` where `status` = 1 && dstatus = 0  ".$searchvar."");  
              ...
              <div class="card-header">
                <div class="row">
                  ...
                  <div class="col-md-4 text-right">
                      <a href="<?php echo SITEADMINURL;?>city/add.php" class="btn btn-info">Add New</a>
                  </div>
                </div>
              </div>
              <!-- /.card-header -->
                ...
                <table class="table table-bordered">
                  <thead>
                    <tr>
                      ...
                      <th style="color: red;">Country Name</th>                      
                      <th style="color: red;">State Name</th>                      
                  ...
                  if($getdata->num_rows>0){
                    while($file = $getdata->fetch_assoc()){
                      $Checkcountry = $conn->query("select name from country where id='".$file['country_id']."'");
                      $fetchCountry = $Checkcountry->fetch_assoc();
                      $Checkstate = $conn->query("select name from state where id='".$file['state_id']."'");
                      $fetchstate = $Checkstate->fetch_assoc();
                     $status = $file['status'];
                      if($status == 1){
                        $status = 'Active';
                      }else{
                    ...
                    <tr>
                      ...
                      <td><?php echo $fetchstate['name'] ; ?></td>  
                      <td><?php echo $fetchCountry['name'] ; ?></td>  
                      <td>
                      <?php echo $statusarr [$file['status']] ; ?>
                      </td>
                       
                        <td>
                        <a href="<?php echo SITEADMINURL;?>city/edit.php?id=<?php echo $file['id'];?>" class="btn btn-success"><i class="fa fa-pencil" aria-hidden="true"></i> Edit</a>
                        <a href="<?php echo SITEADMINURL;?>city/delete.php?id=<?php echo $file['id'];?>" onclick="return confirm('Are You Sure To Delete')" class="btn btn-danger"><i class="fa fa-trash" aria-hidden="true"></i> Delete</a>
