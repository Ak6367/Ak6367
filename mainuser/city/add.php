  ...
  if(isset($_POST['name'])){
    ...
    if($iserror == 0){
      ...
      if($checkCountry->num_rows > 0){
        ...
        $insertQuery = $conn->query("insert into city set name='".$_POST['name']."', status='".$_POST['status']."', country_id ='".$_POST['country']."', state_id='".$_POST['state']."', created_at='".date('Y-m-d H:i:s')."' ");
        if($insertQuery){
                  ...
                  <div class="form-group">
                    ...
                    <select name="country" id="country" onchange="getstate(this.value);" class="form-control custom-select">
                      ...
                      while ($countrydata = $check_country->fetch_assoc()) {
                        // prd($value);
                        $sel = '';
                        if (isset($_POST['country']) && $_POST['country'] == $countrydata['id']) {
                          $sel = 'selected';
                        }
