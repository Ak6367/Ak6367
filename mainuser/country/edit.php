  ...
  if(isset($_POST['name'])){
    ...
    if($iserror == 0){
      ...
      if($checkCountry->num_rows > 0){
        ...
        $insertQuery = $conn->query("update country set name='".$_POST['name']."',updated_at='".date('Y-m-d H:i:s')."' where id='".$countryid."' ");
        if($insertQuery){
          $_SESSION['success_msg'] = ucwords('Country name edited sucessfuly');
          header("location:".SITEADMINURL.'country');
        }
      }
    }
  }
  $getCountry = $conn->query("select * from country where id = '".$countryid."' ");
  if($getCountry->num_rows > 0){
    $fetchCountry = $getCountry->fetch_assoc();
    $Cname = $fetchCountry['name'];
              ...
              <div class="row">
                <div class="col-md-12">
                  <div class="form-group">
                    ...
                    <input type="text" id="name" name="name" value="<?php echo isset($_POST['name']) ? $_POST['name'] : $Cname ?>" class="form-control">
                  </div>
