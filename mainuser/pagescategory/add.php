 ...
 // print_r($_POST);
  $iserror = 1;
    if(isset($_POST['name'])){
      ...
      if($iserror == 1){
        $checkid = $conn->query("select * from page_category where name = '".$_POST['name']."' ");
        if($checkid->num_rows > 0){
          $nameerr = "Page Name Already Taken";
        }else{
          $insert_data = $conn->query("insert into `page_category` set `name` = '".$_POST['name']."', `status` = '".$_POST['status']."', `created_at` = '".date('Y-m-d H:i:s')."' ");
        
        if($insert_data){
          $_SESSION['success_msg'] = 'Pages Category Added Successfully';
