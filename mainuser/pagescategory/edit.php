...
<?php
  //print_r($_SESSION);
 // print_r($_POST);
  $get_user_data = $conn->query("select * from page_category where id = '".$_GET['id']."' ");
  ...
  $iserror = 1;
    if(isset($_POST['name'])){
      ...
      if($iserror == 1){
        $updatedata = $conn->query("update `page_category` set `name` = '".$_POST['name']."', `status` = '".$_POST['status']."', `updated_at` = '".date('Y-m-d H:i:s')."' where id = '".$_GET['id']."'");
        // prd($updatedata);
        if($updatedata){
          $_SESSION['success_msg'] = 'Page Category Updated Successfuly';
        ...
        <div class="col-md-12">
            ...
            <div class="card-body">
              ...
              <div class="form-group text-right">
                ...
                <a href="<?php echo SITEADMINURL;?>pagescategory" onclick="alert('Are You Sure')" class="btn btn-danger">Cancel</a>
              </div>
            </div>
            <!-- /.card-body -->
          </div>
