...
if (isset($_POST['pincode'])) {
  ...
  if (empty($_POST['amount'])) {
    $iserror = 0;
    $amounterr = "Please Select Page Category";
  }

  if ($iserror == 1) {
    //$checkid = $conn->query("select * from shippings where name = '" . $_POST['name'] . "' ");
    // if ($checkid->num_rows > 0) {
    //   $nameerr = "Bannner Name Already Taken";
    // } else {
      $insert_data = $conn->query("insert into `shippings` set `pincode` = '" . $_POST['pincode'] . "', amount='" . $_POST['amount'] . "', `created_at` = '" . date('Y-m-d H:i:s') . "' ");

      if ($insert_data) {
        $_SESSION['success_msg'] = 'Shippings Added Successfully';
              ...
              <div class="row">
                <div class="col-md-6">
                  <div class="form-group">
                    ...
                    <input type="text" id="pincode" name="pincode" value="<?php echo isset($_POST['pincode']) ? $_POST['pincode'] : '' ?>" class="form-control">
                  </div>
