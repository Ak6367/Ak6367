...
if (isset($_POST['name'])) {
  ...
  if ($iserror == 1) {
    $checkid = $conn->query("select * from brands where name = '" . $_POST['name'] . "' ");
    if ($checkid->num_rows > 0) {
      $nameerr = "Brand Name Already Taken";
    } else {
      $brandimg = '';
      $brandimgname = 'Brand_image_' . time() . '_' . rand(1, 9999999) . '.' . $ext;
      if (move_uploaded_file($_FILES['images']['tmp_name'], '../../../upload/brands/' . $brandimgname)) {
        $brandimg = $brandimgname;
      }
      $insert_data = $conn->query("insert into `brands` set `name` = '" . $_POST['name'] . "', `status` = '" . $_POST['status'] . "', `created_at` = '" . date('Y-m-d H:i:s') . "', `image` = '" . $brandimg . "' ");
