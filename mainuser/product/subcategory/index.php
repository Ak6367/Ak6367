...
$getdata = $conn->query("select * from `subcategories` where `dstatus` = 0 ".$searchvar."  order by id desc LIMIT $start_from, $per_page_record ");
// print_r($getdata);die;

$chekdata =$conn->query("select * from `subcategories` where `dstatus` = 0 ".$searchvar."");  
              ...
              <div class="card-header">
                <div class="row">
                  ...
                  <div class="col-md-4 text-right">
                      <a href="<?php echo SITEADMINURL;?>product/subcategory/add.php" class="btn btn-info">Add New</a>
                  </div>
                </div>
              </div>
              <!-- /.card-header -->
                ...
                <table class="table table-bordered">
                  ...
                  if($getdata->num_rows>0){
                    while($file = $getdata->fetch_assoc()){
                      $sql_cate = $conn->query("select * from categories where id = '".$file['category_id']."' ");
                      $fetch_cate = $sql_cate->fetch_assoc();
                      // prd($sql_cate);
                     $status = $file['status'];
                      if($status == 1){
                        $status = 'Active';
                    ...
                    <tr>
                      ...
                      <td><?php echo $file['name'] ; ?> (<?php echo $fetch_cate['name'];?>)</td>
                      
                      <td>
                      <?php echo $statusarr [$file['status']] ; ?>
                      </td>
                       <td>
                            <img src="<?php echo SITEURL;?>upload/subcategory/<?php echo $file['image'];?>" width="50%">
                        </td>
                        <td>
                        <a href="<?php echo SITEADMINURL;?>product/subcategory/edit.php?id=<?php echo $file['id'];?>" class="btn btn-success"><i class="fa fa-pencil" aria-hidden="true"></i> Edit</a>
                        <a href="<?php echo SITEADMINURL;?>product/subcategory/delete.php?id=<?php echo $file['id'];?>" onclick="return confirm('Are You Sure0 To Delete')" class="btn btn-danger"><i class="fa fa-trash" aria-hidden="true"></i> Delete</a>
