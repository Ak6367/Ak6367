...
if (isset($_POST['name'])) {
  ...
  if ($iserror == 1) {
    $checkid = $conn->query("select * from subcategories where name = '" . $_POST['name'] . "' ");
    if ($checkid->num_rows > 0) {
      $nameerr = "Brand Name Already Taken";
    } else {
      $upload_img = '';
      $imgname = 'Brand_image_' . time() . '_' . rand(1, 9999999) . '.' . $ext;
      if (move_uploaded_file($_FILES['images']['tmp_name'], '../../../upload/subcategory/' . $imgname)) {
        $upload_img = $imgname;
      }
      $insert_data = $conn->query("insert into `subcategories` set `name` = '" . $_POST['name'] . "', category_id = '".$_POST['category']."', `status` = '" . $_POST['status'] . "', `created_at` = '" . date('Y-m-d H:i:s') . "', `image` = '" . $upload_img . "' ");
...
<script>
  function checkvalidation() {
    ...
    // alert(status);
    if (name == '') {
      document.getElementById('error_name').innerHTML = 'Please Enter Sub Category Name';
      iserror = 0;
    } else {
      document.getElementById('error_name').innerHTML = '';
    }
    if (status == '') {
      document.getElementById('error_status').innerHTML = 'Please Select Sub Category Status';
      iserror = 0;
    } else {
