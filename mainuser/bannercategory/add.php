 ...
 // print_r($_POST);
  $iserror = 1;
    if(isset($_POST['name'])){
      ...
      if($iserror == 1){
        $checkid = $conn->query("select * from banner_categories where name = '".$_POST['name']."' ");
        if($checkid->num_rows > 0){
          $nameerr = "Bannner Name Already Taken";
        }else{
          $insert_data = $conn->query("insert into `banner_categories` set `name` = '".$_POST['name']."', `status` = '".$_POST['status']."', `created_at` = '".date('Y-m-d H:i:s')."' ");
