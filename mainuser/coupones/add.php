...
if (isset($_POST['coupones'])) {
  ...
  if ($iserror == 1) {
    $checksql = $conn->query("select * from coupones where code='".$_POST['code']."' ");
    if($checksql->num_rows > 0){
      $codeerr = 'Coupon Code Already Taken';
    }else{
      $insert_data = $conn->query("insert into `coupones` set `code` = '" . $_POST['code'] . "', user_id='" . $_POST['coupones'] . "', `type` = '" . $_POST['type'] . "', value   = '" . $_POST['value'] . "', start_date = '".$_POST['startdate']."', end_date = '".$_POST['enddate']."', is_multiple = '".$_POST['multiple']."', `created_at` = '" . date('Y-m-d H:i:s') . "'");

      if ($insert_data) {
        $_SESSION['success_msg'] = 'Banner Added Successfully';
        header("location:" . SITEADMINURL . 'coupones');
      } else {
        $codeerr = "Unable To Process";
      }
    }
              ...
              <div class="row">
                ...
                <div class="col-md-6">
                  <div class="form-group">
                    ...
                    <select name="coupones" id="coupones" class="form-control custom-select">
                      ...
                      <option value="0" <?php echo isset($_POST['coupones']) && $_POST['coupones'] == 0 ?'selected':'';?>>All User</option>
                      <?php 
                        ...
                        while($front_user_data = $front_user->fetch_array()){
                          $select ='';
                          if($_POST['coupones']== $front_user_data['id']){
                            $select = 'selected';
                          }
                ...
                <div class="col-md-6">
                  ...
                  <select name="type" id="type" class="form-control custom-select" onchange="checkvalue();">
                      ...
                      <option value="1" <?php echo isset($_POST['type']) && $_POST['type'] == 1?'selected':'';?>>Percentage</option>
                      <option value="2"<?php echo isset($_POST['type']) && $_POST['type'] == 2?'selected':'';?>>Fixed</option>
                      </option>
                    </select>
                  </div>
                ...
                <div class="col-md-6">
                  <div class="form-group">
                    ...
                    <input type="text" name="value" id="value" value="<?php echo isset($_POST['value'])?$_POST['value']:'';?>"  class="form-control" onchange="checkvalue();">
                  </div>
                ...
                <div class="col-md-6">
                  <div class="form-group">
                    ...
                    <input type="date" name="startdate" id="startdate" value="<?php echo isset($_POST['startdate'])?$_POST['startdate']:''; ?>"  class="form-control">
                  </div>
                ...
                <div class="col-md-6">
                  ...
                  <select name="multiple" id="multiple" class="form-control custom-select">
                      <option value="">Select Coupone Type</option>
                      <option value="0" <?php echo isset($_POST['multiple']) && $_POST['multiple'] == 0?'selected':'';?>>Yes</option>
                      <option value="1"<?php echo isset($_POST['multiple']) && $_POST['multiple'] == 1?'selected':'';?>>No</option>
                      </option>
                    </select>
                  </div>
